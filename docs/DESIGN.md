...
- **Progress is a projection, not a record (D19).** The journal (program counter) and ledger (every atomic action) *already* hold everything a progress report needs, so progress is never accumulated by hand — it is a **pure projection** of `work-orders ∪ journal ∪ ledger ∪ inbox` into a nested tree (effort → vertical slice → work order → atomic action), regenerated **with no model in the loop** by `lib/progress.mjs` on every journal/ledger write (a `PostToolUse` hook), and written to the **presentation mirror** `.reasonable/progress.{json,md}` — **effort-scoped** (this effort's own `.reasonable/`; the hook resolves the effort from the *changed artifact's* path, never cwd, so several efforts in one repo never cross-write). The human pins `progress.md` to watch a multi-hour run live (it grows itself as work reveals scope, and reflects parallel lanes, because each render just reads current disk truth); `progress.json` feeds a graphical view. Because it is mechanical, progress reporting costs **zero tokens** and can never drift from the ledger it is computed from; because the mirror is a *separate* artifact with its own *single* deterministic writer, the lone-scribe rule (D3b) is untouched. A second **ephemeral live tier** (`progress-live.jsonl`, tier-2) overlays per-tool-call heartbeats so a multi-agent wave is never frozen on `pending` between scribe writes: each work order renders its fixed **pipeline scaffold** (`provision → … → audit`, done/active/pending) with the live tool folded into the active stage and the acting agent's **own todo list** nested beneath it; atomic-action lines are ordered by `seq` (causal order, never by `ts`) and carry literal `[HH:MM:SS]` timestamps, with any `ts` that contradicts `seq` suppressed as untrustworthy. (Why not the model, or a timer-driven digest? An LLM tracker would burn tokens and could lie; the native session-task store can't be rebuilt from the ledger and isn't readable from a hook — only a ledger-derived file honors both *deterministic* and *crash-recoverable*.)
...
- **Run mode** — gated (`reasonable:develop`, default — gates block) vs autonomous (`reasonable:develop-autonomously` — gates self-ratify and log, never block, no step skipped). The outermost control axis; chosen only by the entry skill, never inferred from a standing directive.
- **Supervision profile** — strict / standard / trusting dial; the finer control nested inside gated mode. Initial profile set by the entry skill (`develop`→strict, `develop-autonomously`→trusting); lower-level phases never override it; tuned at vertical-slice retro. No profile waives a mechanical check.
