...
| ledger.jsonl `verifier-verdict` line | **adversary** proposes (read-only, returns it as data); the narrow **verdict-writer** appends it | via the **ledger controller CLI**, content-referencing the judged commit — **not** a git commit of state (D4/D5). Since reasonable 3.0 A3b, **verdict-writer** also lands `atom-transitioned` and `atom-verdict` lifecycle events via the same command. |
