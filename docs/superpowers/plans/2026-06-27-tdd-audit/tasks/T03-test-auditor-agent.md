# Task T03: test-auditor agent

## References
- Read: `../shared/architecture.md`
- Read: `../shared/interfaces.md` (§1 the CLI, §2 the agent identity, §4 the rubric path)
- Read for voice + allowlist discipline: `agents/auditor.md`, `agents/census.md`,
  `agents/blind-test-writer.md`
- The rubric it cites: `skills/tdd-audit/references/test-honesty-rubric.md` (from T02)

## Dependencies
- Depends on: T02 (the rubric must exist to be cited)
- Depended on by: T04 (`agentType: 'reasonable:test-auditor'`)
...
- Frontmatter `tools:` MUST be exactly `Read, Grep, Glob, Bash` — **no** `Edit`/`Write`. This is the
  capability-enforced audit-independence guarantee; treat it as load-bearing.
- The body must define the seven lenses and what each returns, point honesty/confirm at the
  canonical rubric, and give the exact `confirm` command from `interfaces.md §1`.
- Make read-only explicit: it reports; it never fixes, never edits tests or source.

## Negative Constraints (DO NOT)
- Do NOT add `Edit` or `Write` to the allowlist.
- Do NOT duplicate the rubric text here — cite the file (DRY; T02 owns it).
- Do NOT have it write `.reasonable/` state — this agent runs standalone too.

## Implementation Steps

### Step 1: Create the agent constitution
...
feat(tdd-audit): add read-only test-auditor agent (lens-parameterized)
...
## Acceptance Criteria
- [ ] `agents/test-auditor.md` exists with `tools: Read, Grep, Glob, Bash` (no Edit/Write).
- [ ] All seven lenses are defined; honesty + confirm cite the canonical rubric path.
- [ ] The confirm lens gives the exact reverse-discriminator command from `interfaces.md §1`.
- [ ] The rubric text is cited, not duplicated.
