# T03b — Layered-DAG layout implementation (green)

**role:** green
**Depends on:** T03a
**Owns (stage only these):** `lib/topology-view.mjs`

> **Read first:** `../shared/interfaces.md` (§B1), `../shared/conventions.md` (module system, purity,
> the append-marker), `../knowledge/running-tests.md`. You are the `green` role: **make the locked
> T03a tests pass. Write NO tests.**
>
> **Scope / Negative Constraints:**
> - `test/topology-layout.test.mjs` is **READ-ONLY** — do not modify it. If a test looks wrong, STOP and
>   escalate to the supervisor; never edit a locked test to make it pass.
> - Create **only** `lib/topology-view.mjs`. Do not touch any other `lib/*.mjs`, any `agents/*.md`, any
>   doc, or the roadmap. **Import only `{ liftEdges } from './graph.mjs'`** — nothing else (no
>   `legibility.mjs`, no `policy.mjs`/`goals.mjs`, no `node:fs`). Law 1.
> - End the file with the **append marker** (below) so T04b appends `renderTopologyHtml` beneath it —
>   leave nothing after the marker.

**Files:**
- Create: `lib/topology-view.mjs`

- [ ] **Step 1: Create `lib/topology-view.mjs` with the layout half**

Write `lib/topology-view.mjs` with exactly this content:
...
- [ ] **Step 2: Run the layout tests to verify they pass**

Run: `node test/topology-layout.test.mjs`
Expected: `topology-layout: all N checks pass. ✓` — every rank-consistency, crossing-reduction,
determinism, totality, coordinate, cycle-safety, and degenerate check green. If the "X" fixture still
crosses, the barycenter sweep is not reordering rank 1 — re-check that `sweep` updates `orderOf`
immediately per rank and that the down sweep uses `preds`.

- [ ] **Step 3: Run the whole suite to confirm zero regressions**

Run: `for t in test/*.test.mjs; do node "$t"; done`
Expected: no `FAIL` line anywhere. P6e adds a new file with one import (`liftEdges`, already exported by
`graph.mjs`); it changes no existing behavior, so every prior file stays green.

- [ ] **Step 4: Commit**

```bash
git add lib/topology-view.mjs
git commit -m "feat(topology-view): layered-DAG layout — longest-path ranks + barycenter ordering (green, P6e)"
```

**Do not implement `renderTopologyHtml` here.** T04b appends it below the marker. Leave the marker as the
file's last line.
