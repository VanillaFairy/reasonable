# T01 — Author the topologist constitution

**role:** author
**Depends on:** —
**Owns (stage only this):** `agents/topologist.md`

> **Read first:** `../shared/interfaces.md` (§A — the frontmatter allowlist, the five §5.1 outputs, the
> post-genesis remit, what it must NOT do), `../shared/conventions.md` (the authoring discipline —
> mirror `route-planner.md`), and the **shipped** `agents/route-planner.md` (the direct template — its
> frontmatter, structure, and how the allowlist *is* the enforcement) + `agents/blind-test-writer.md`
> (the canonical "no Bash = bias-prevention-by-capability" example). This is **not** a red/green task —
> it is a document. Author it faithfully; T02 (a fresh read-only subagent) audits the allowlist.
>
> **The one load-bearing line is `tools: Read, Grep, Glob`** — verbatim `route-planner`'s allowlist. It
> is what makes "the topologist PROPOSES `goals.json`/`policy.json`, it cannot write them" true **by
> capability**. Do not add `Write`/`Edit`/`Bash`. (See plan Flag 1.)

**Files:**
- Create: `agents/topologist.md`

- [ ] **Step 1: Write the constitution**

Write `agents/topologist.md` with exactly this content:
...
- [ ] **Step 2: Sanity-read against the template**

Open `agents/route-planner.md` beside your file and confirm, line by line, that the **frontmatter shape
matches** (`name` / `description` / `model` / `tools`) and the **allowlist is identical** (`tools: Read,
Grep, Glob`). Confirm no `Write` / `Edit` / `Bash` appears anywhere in the frontmatter. Confirm the body
never claims a capability the allowlist denies (no "I write", "I run", "I append", "I persist" as a
*self* action — always "I propose / the orchestrator persists"). Do **not** run a test — there is none.

- [ ] **Step 3: Commit**

```bash
git add agents/topologist.md
git commit -m "feat(topologist): genesis topology role — five §5.1 outputs, propose-not-write (read-only allowlist, P6e)"
```

**Do not touch any other file.** T02 (a fresh read-only subagent) audits this constitution's allowlist and
scope next.
