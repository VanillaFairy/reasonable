# T03a — Layered-DAG layout tests (red)

**role:** red
**Depends on:** —
**Owns (stage only these):** `test/topology-layout.test.mjs`

> **Read first:** `../shared/interfaces.md` (§B1 — the `layoutTopology` signature, longest-path ranking,
> barycenter cross-reduction, coordinates, cycle-safety, degenerate input), `../shared/conventions.md`
> (the standalone-Node pure harness; assert **properties over real overlapping-edge fixtures**, not
> coordinate goldens), `../knowledge/running-tests.md`. You are the `red` role: **write the failing tests
> only. Do not implement `lib/topology-view.mjs`.** Pin the layout's §5.3 *properties* — rank-consistency,
> determinism, no node loss, **crossing reduction**, cycle-safety, coordinate monotonicity — never a
> golden coordinate the design leaves open (plan Flag 4).

**Files:**
- Create: `test/topology-layout.test.mjs`

- [ ] **Step 1: Write the failing test file**

Write `test/topology-layout.test.mjs` with exactly this content:
...
Expected: **the import fails** because `lib/topology-view.mjs` does not exist yet — a module-not-found
error (`Cannot find module '../lib/topology-view.mjs'`). This is the correct red: the tests fail because
the module is absent, not because an assertion is wrong.

- [ ] **Step 3: Commit**

```bash
git add test/topology-layout.test.mjs
git commit -m "test(topology-view): layered-DAG layout — rank-consistency + crossing reduction + cycle-safety (red, P6e)"
```

**Do not implement anything.** The `green` task (T03b) makes these pass.
