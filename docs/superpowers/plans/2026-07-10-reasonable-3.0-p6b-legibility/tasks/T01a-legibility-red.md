# T01a — Legibility-law tests (red)

**role:** red
**Depends on:** —
**Owns (stage only these):** `test/legibility.test.mjs`

> **Read first:** `../shared/interfaces.md` (the two signatures, the finding grammar, each measure's
> exact formula, the guard's load-bearing property), `../shared/conventions.md` (the standalone-Node
> pure-fixture harness), `../knowledge/running-tests.md`. You are the `red` role: **write the failing
> tests only. Do not implement `lib/legibility.mjs`.** Assert §5.2's intent (width / tangle / coupling
> / chain, and the density-reduction guard); where `interfaces.md` flags a metric as contestable
> (cross-cone density), assert the *decision* under thresholds you choose, not an over-fitted golden.

**Files:**
- Create: `test/legibility.test.mjs`

- [ ] **Step 1: Write the failing test file**

Write `test/legibility.test.mjs` with exactly this content:
...
Expected: **the import fails** because `lib/legibility.mjs` does not exist yet — a module-not-found
error (`Cannot find module '../lib/legibility.mjs'`). This is the correct red: the tests fail because
the module is absent, not because an assertion is wrong.

- [ ] **Step 3: Commit**

```bash
git add test/legibility.test.mjs
git commit -m "test(legibility): bounded width/tangle/coupling/chain + R8 density guard (red, P6b)"
```
