# T01a — goals.json loader tests (red)

**role:** red
**Depends on:** —
**Owns (stage only these):** `test/goals-loader.test.mjs`

> **Read first:** `../shared/interfaces.md` (the `readGoals` contract + the `scenarioCitations`
> grounding), `../shared/conventions.md` (the `route.test.mjs`-style temp-dir harness; validate
> diagnostic SHAPE never text), `../knowledge/running-tests.md`. You are the `red` role: **write the
> failing tests only. Do not implement `readGoals`.** Pin the conservative-loader contract (absent /
> malformed / valid, all-or-nothing, degrade-to-null, shape-not-value); do not pin diagnostic strings.

**Files:**
- Create: `test/goals-loader.test.mjs`

- [ ] **Step 1: Write the failing test file**

Write `test/goals-loader.test.mjs` with exactly this content:
...
Expected: **the import fails** because `lib/goals.mjs` does not exist yet — a module-not-found error
(`Cannot find module '.../lib/goals.mjs'`). That is the correct red: the tests fail because the loader
does not exist, not because an assertion is wrong.

- [ ] **Step 3: Commit**

```bash
git add test/goals-loader.test.mjs
git commit -m "test(goals): conservative goals.json loader — absent/malformed/valid + servesEdges compose (red, P6d)"
```

**Do not implement anything.** The `green` task (T01b) creates `lib/goals.mjs`.
