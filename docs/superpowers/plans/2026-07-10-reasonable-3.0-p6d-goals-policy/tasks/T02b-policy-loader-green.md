# T02b — policy.json loader impl (green)

**role:** green
**Depends on:** T02a
**Owns (stage only these):** `lib/policy.mjs`

> **Read first:** `../shared/interfaces.md`, `../shared/conventions.md`. You are the `green` role:
> **make the locked tests pass; write no tests.** `test/policy-loader.test.mjs` is **READ-ONLY — do not
> modify it.** If a test looks wrong, STOP and escalate to the supervisor; do not edit it.
...
Read `test/policy-loader.test.mjs` end to end. Note: `policy.json` is an **object** with an OPEN field
set; the return is `{ policy, diagnostic }`; on success the parsed object is returned **verbatim**
(unknown keys survive); validation is **shape, never value** (absurd numbers load clean); the four
required sub-shapes are `weights` / `legibility` / `cadence` / `dials`, checked in that order.

- [ ] **Step 2: Create `lib/policy.mjs`**

Write `lib/policy.mjs` with exactly this content:
...
Expected: `policy: all N checks passed. ✓` (no `FAIL` line, exit 0).

- [ ] **Step 4: Run the full suite to confirm zero regressions**

Run: `for t in test/*.test.mjs; do node "$t"; done`
Expected: no `FAIL` line anywhere — this part is purely additive (a brand-new file).

- [ ] **Step 5: Commit**

```bash
git add lib/policy.mjs
git commit -m "feat(policy): readPolicy — conservative policy.json loader (green, P6d)"
```

**Do not modify the test file, touch `lib/goals.mjs` (T01b's), `docs/`, the roadmap, `plugin.json`, or
the README.** Docs are T03; the roadmap status cell is T04.
