...
- **Phase skills** (orchestration checklists, run in the main session): `using-reasonable` (entry +
  triage), `analysis`, `scaffolding`, `vertical-slice-execution`, `retro`.
...
Start any greenfield effort by invoking the **`using-reasonable`** skill — it triages applicability
(the methodology engages when *topology is novel* OR *decomposition is uncertain* OR *work spans ≥2
seams*; otherwise it routes you to a lighter path) and then walks `analysis → scaffolding →
vertical-slice-execution → retro`, looping the last two per vertical slice until the route is empty.
