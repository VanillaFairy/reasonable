...
// A missing-signature config is still UNAMBIGUOUSLY identifiable when the effort's own name survives in
// journal.effort (a pre-signature effort that simply predates the field) — reconcile reconstructs the
// pointer from it automatically (§6.1). Only a corrupt config, or a missing-signature with no recoverable
// name, is genuinely unidentifiable.
function hasRecoverableName(root) {
  const j = readJson(join(root, '.reasonable', 'journal.json'));
  return !!(j && typeof j.effort === 'string' && j.effort.trim());
}

function cheapSummary(root) {
  const bs = effortBirthState(root);
  if (bs.state === 'corrupt' || (bs.state === 'missing-signature' && !hasRecoverableName(root))) {
    const why = bs.state === 'corrupt'
      ? `unparseable${bs.reason ? `: ${bs.reason}` : ''}`
      : 'no "effort" name, none recoverable from journal';
    return `• ${basename(root)}: ⚠️ FLAGGED — config unidentifiable (${why}); ` +
      `reconcile HALTs until .reasonable/config.json is fixed.`;
  }
...
function birthFlag(root) {
  const bs = effortBirthState(root);
  if (bs.state === 'missing-signature') {
    // Recoverable from journal.effort → reconcile reconstructs the pointer automatically on this same
    // resolved path; no alarm, no operator action.
    if (hasRecoverableName(root)) return '';
    return `\n⚠️ CONFIG UNIDENTIFIABLE — this effort's .reasonable/config.json has no "effort" name and none ` +
      `is recoverable from journal.effort. reconcile will HALT until you add "effort": "<name>".`;
  }
  if (bs.state === 'corrupt') {
    return `\n⚠️ CONFIG UNPARSEABLE — this effort's .reasonable/config.json does not parse` +
      `${bs.reason ? ` (${bs.reason})` : ''}. reconcile will HALT until you repair the JSON.`;
  }
  return '';
}
