...
function birthFlag(root) {
  ...
  if (bs.state === 'missing-signature') {
    return `\n⚠️ CONFIG MISSING BIRTH SIGNATURE — this effort's .reasonable/config.json has no non-empty ` +
      `"effort" name (often an effort that predates the signature). reconcile will HALT until you add it: ` +
      `"effort": "<name>".`;
  }
  if (bs.state === 'corrupt') {
