...
import { resetLive } from './progress-live.mjs';
...
let context =
  ...
  '  • `reasonable:develop` — GATED (default): every ratification gate blocks for explicit human approval.\n' +
  '  • `reasonable:develop-autonomously` — AUTONOMOUS: gates self-ratify and are logged; never blocks — but ' +
...
if (effortRoot) {
  ...
  // Cold restart resets the EPHEMERAL live channel (D19 tier-2): heartbeats from the
  // pre-restart session are stale "now" noise, so truncate them and regenerate the mirror
  // from canonical truth. Fail-open — the live channel is presentation-only.
  try {
    resetLive(effortRoot);
    const p = await import('./progress.mjs');
    try { p.writeMirror(effortRoot); } catch { /* fail open */ }
  } catch { /* fail open */ }
  try {
