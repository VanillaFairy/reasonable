...
function runAbsence() {
  ...
  for (const rel of changed) {
    ...
    if (c) cmds.add(c); else unresolved.push(rel);
  }
  // EVERY changed test file must resolve, not just one: a changed test no stack can run would
  // be overlaid but never executed — an unverified test hiding behind a sibling's red
  // (docs/artifacts.md: a file matching no stack entry is a LOUD gap, never a silent skip).
  if (unresolved.length) fail(`No test command resolves for changed test file(s) [${unresolved.join(', ')}] — check config.json testCommand stack globs.`);
...
function runReverse() {
  ...
  // stack that OWNS the locus (multi-stack; the locus glob is matched as a literal path prefix
  // against the stack globs, which works for the prefix-style globs the binding tables use).
  // A string testOneCommand / --test-one-cmd is one globless stack, so single-stack resolves
  // to it regardless of locus. A locus spanning MULTIPLE stacks cannot be verified by one
  // one-test command — mutants in the other stack would run under the wrong runner and pass
  // vacuously (a silent wrong-stack run) — so it fails loud instead: one test lives in one
  // stack; split the clause locus per stack.
  const oneTemplates = new Set();
  for (const g of locus) { const c = testOneCommandFor(cfg, g); if (c) oneTemplates.add(c); }
  if (oneTemplates.size > 1) fail(`--reverse locus spans multiple stacks (${oneTemplates.size} distinct testOneCommand entries resolve for [${locus.join(', ')}]) — one characterization test runs under ONE stack; split the locus per stack.`);
  const oneTemplate = oneTemplates.size ? [...oneTemplates][0] : null;
