...
import { buildTree } from './progress-map.mjs';
...
// Windows path comparison is case-INSENSITIVE, and `path.resolve()` PRESERVES whatever
// drive-letter case it was handed (it never touches the FS to canonicalize it). So a
// lowercase `--root c:/…` resolves to "c:/…" while `git worktree list` reports git's stored
// "C:/…" — a plain `===`/`startsWith` then wrongly says they differ, and the worktree scan
// goes blind. Fold case for COMPARISON ONLY (never for a path we hand to git / existsSync /
// slice), and ONLY on win32 (POSIX paths are genuinely case-sensitive). Both args are already
// norm()'d (forward slashes) by the callers.
const foldPath = (p) => (process.platform === 'win32' ? String(p).toLowerCase() : String(p));
const samePath = (a, b) => foldPath(a) === foldPath(b);
const underPath = (base, p) => foldPath(p).startsWith(foldPath(base) + '/');

// §5.1 cross-check: the ledger fold (lib/wo-status.mjs) is the AUTHORITATIVE WO status; the journal's
// per-WO `status` is a cache reconcile only cross-checks and warns on. The two carry different
// vocabularies, so a disagreement is judged by mapping the well-known journal statuses onto the fold's
// five values; an unrecognized journal status (e.g. legacy 'dead-end', handled by deadEndSet) maps to
// null and is simply not cross-checked — conservative, never a false-alarm note.
...
export function reconcile(effortRoot) {
  ...
  // from journal.workOrders. The journal is a cache cross-checked below; a WO living only in the
  // ledger (the invisible-WO incident) is surfaced after the per-WO loop instead of being dropped.
  ...
  for (const [id, wo] of Object.entries(workOrders)) {
    // The ledger fold is the authoritative status; the journal is cross-checked, never trusted.
    ...
    if (foldStatus !== null) {
      ...
      if (je !== null && je !== foldStatus) {
        notes.push(`WO ${id}: journal status '${wo.status}' disagrees with ledger fold '${foldStatus}' — using ledger.`);
      }
    }
    // Crash recovery examines a LIVE work order. The ledger fold WINS: a fold of `done`/`blocked`/
    // `dropped` is decisive — never re-open or downgrade a WO the ledger already settled, even if the
    // journal still says `dispatched`. It runs when the fold says RUNNING, or when the fold is
    // absent/`pending` AND the journal still claims dispatched/checkpointed — the journal's claim is
    // then verified against git + ledger below (an unbacked claim downgrades, preserving the D8b
    // partition and the journal-only recovery path, incl. the idempotent post-downgrade re-affirm).
    const journalClaimsLive = wo.status === 'dispatched' || wo.status === 'checkpointed';
    const live = foldStatus === 'running'
      || ((foldStatus === null || foldStatus === 'pending') && journalClaimsLive);
    if (!live) continue;
    ...
    if (!wtExists && ahead === 0 && wo.status === 'checkpointed') {
      ...
      // Dispatched with no work landed: safe to downgrade, loses no truth. Mutate ONLY
      // `status` — `dispatchEpoch` (and every other field) must survive, so the next
      // write-ahead lift bumps it from the right base and the crash boundary renders (D19).
      wo.status = 'pending';
  ...
  // changes — that stays redispatch-guard.mjs's job. Two on-disk shapes both
  // count: the canonical `status:"merged"` (docs/artifacts.md), and the
  // `status:"green", merged:true` shape the orchestrator actually writes once a
  // vertical-slice gate lands (skills/vertical-slice-execution/SKILL.md §7).
  const terminalWorkOrders = Object.entries(workOrders)
    .filter(([, wo]) => wo && (wo.status === 'merged' || wo.merged === true))
    .map(([id]) => id);
