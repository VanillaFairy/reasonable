...
//                     a ledger entry with no commit; two lanes claiming one work
//                     order; an unaccounted floor-test change (BF8); an absent
//                     `config.runMode` on a cold restart (D10). Any AMBIGUOUS sets
//                     result.halt = true with a haltReason + evidence and is
//                     surfaced to the human as a BLOCKING decision — never a guess.
//
...
import { findEffortRoot, readJson, readJsonl, gitTry, norm, loadConfig } from './effort.mjs';
...
export function reconcile(effortRoot) {
  ...
  for (const [id, wo] of Object.entries(workOrders)) {
    ...
    const ahead = hasBranch ? commitsAhead(effortRoot, branch) : 0;
    const wtExists = wo.worktree && existsSync(join(effortRoot, wo.worktree));
    ...
    const checkpointCommit = hasBranch ? trailedCheckpoint(effortRoot, branch, id) : null;
    ...
    if (!wtExists && ahead === 0 && wo.status === 'checkpointed') {
      ...
      const claim = reclaimOrphanCommits(effortRoot, id, branch, wo, ledger);
  ...
  for (const wt of lanesOnDisk) {
    const rel = norm(wt.replace(effortRoot, '').replace(/^[/\\]+/, ''));
    const known = Object.keys(journalLanes).some((l) => norm(l) === rel) ||
      Object.values(workOrders).some((w) => norm(w.worktree || '') === rel);
    if (!known) {
      ...
      const claim = accountOrphanWorktree(effortRoot, rel, orphanBranch, workOrders, ledger);
      if (claim.class === 'AMBIGUOUS') {
        ambiguities.push({ haltReason: claim.haltReason, evidence: claim.evidence });
  ...
  for (const lanePath of Object.keys(journalLanes)) {
    if (!existsSync(join(effortRoot, lanePath))) {
  ...
  // --- Floor-integrity pass (BF8 / D-BF8) — a SEPARATE reconcile pass. ------
  // Floor integrity is a test-set property, distinct from the commit-only D8b
  // partition. An unaccounted floor change (no characterization-promotion /
  // change-characterized / declared floorImpact event explains it) is AMBIGUOUS.
  const floor = floorIntegrity(effortRoot);
  const floorAmbiguous = floor.filter((t) => t.ambiguous);
  for (const t of floorAmbiguous) {
    const changedFiles = t.files.filter((f) => f.changed).map((f) => f.path);
    ambiguities.push({
      haltReason: `floor test "${t.id}" changed with no accounting event`,
      evidence: { floorTest: t.id, locus: t.locus, changedFiles },
    });
    notes.push(`Floor test "${t.id}": ${changedFiles.join(', ')} changed with no characterization-promotion / change-characterized / floorImpact event → AMBIGUOUS (regression-floor breach).`);
  }
  ...
  const openInbox = (Array.isArray(inbox) ? inbox : []).filter((i) => i.status !== 'resolved');

  const halt = ambiguities.length > 0;
  const result = {
    active: true,
    ...
    floorIntegrity: { checked: floor.length, ambiguous: floorAmbiguous.length },
    staleness,
    openInbox,
    halt,
  };
  if (halt) {
    result.haltReason = ambiguities.map((a) => a.haltReason).join('; ');
 ...
 */
function reclaimOrphanCommits(effortRoot, woId, branch, wo, ledger) {
  const recorded = new Set((wo.commits || []).map(normSha));
  const heads = laneCommits(effortRoot, branch);
 ...
 */
function accountOrphanWorktree(effortRoot, rel, branch, workOrders, ledger) {
  const heads = branch ? laneCommits(effortRoot, branch) : [];
  for (const c of heads) {
    const trailerWO = workOrderTrailer(effortRoot, c);
    if (trailerWO) {
...
/** Commits on `branch` not on the default base, oldest→newest, as full SHAs. */
function laneCommits(effortRoot, branch) {
 ...
 */
function trailedCheckpoint(effortRoot, branch, woId) {
  const heads = laneCommits(effortRoot, branch);
...
function listWorktrees(effortRoot) {
  ...
  return r.out.split(/\r?\n/).filter((l) => l.startsWith('worktree ')).map((l) => l.slice('worktree '.length).trim())
    .filter((p) => norm(p) !== norm(gitTry(['rev-parse', '--show-toplevel'], effortRoot).out.trim()));
}

function commitsAhead(effortRoot, branch) {
  // commits on `branch` not on the default branch (best-effort: vs origin/HEAD or main/master).
  for (const base of ['origin/HEAD', 'main', 'master']) {
...
export function briefing(r) {
  ...
  if (r.floorIntegrity) {
    lines.push(`Floor integrity: ${r.floorIntegrity.checked} floor test(s) checked, ${r.floorIntegrity.ambiguous} unaccounted change(s).`);
  }
