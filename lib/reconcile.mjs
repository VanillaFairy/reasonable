...
export function reconcile(effortRoot) {
  ...
  if (birth.state === 'corrupt' || birth.state === 'missing-signature') {
    ambiguities.push({
      haltReason: `config.json birth-state is '${birth.state}' — a torn/foreign effort config may not be trusted at recovery`,
      evidence: { effortBirthState: birth.state, reason: birth.reason ?? null },
    });
    notes.push(`config.json birth-state '${birth.state}'${birth.reason ? ` (${birth.reason})` : ''} → AMBIGUOUS (missing/torn birth signature; a human must repair .reasonable/config.json before recovery proceeds).`);
  }
