...
export function reconcile(effortRoot) {
  ...
  for (const [id, wo] of Object.entries(workOrders)) {
    ...
    if (!wtExists && ahead === 0 && wo.status === 'checkpointed') {
      ...
      // same event would produce (pending, detail "downgraded — awaiting redispatch" — see
      // progress-map.mjs's node-downgraded mapping), the downgrade was already recorded and
      // nothing has redispatched it since, so appending again would just be a duplicate claim.
      const existing = findById(buildTree(effortRoot), id);
      const alreadyRecorded = existing
        && existing.node.status === 'pending'
        && existing.node.detail === 'downgraded — awaiting redispatch';
      if (!alreadyRecorded) {
