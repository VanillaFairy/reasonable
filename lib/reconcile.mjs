...
//
// The EPHEMERAL live channel (D19 tier-2, .reasonable/progress-live.jsonl) is NOT
// reconciled here and reconcile() does NOT write it: this function also runs MID-RUN
// (the runner's reconciler prologue), where wiping live heartbeats from active agents
// would be wrong. Its reset is owned by the COLD-RESTART path (session-start truncates
// it) and the projection ignores any heartbeat older than journal.lastReconciled — so a
// reconcile naturally resets the "now" view without a write side-effect leaking in here.
//
