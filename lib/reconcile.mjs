...
// test and a second consumer (the vertical-slice runner per-work-order
// distributeStaleness). reconcile imports trustStaleness (top of file) and threads its
// result into the briefing; the derived flat staleTrusted id list (assembled in
// reconcile()) is what the reconciler copies verbatim, so no agent re-derives it in prose.
