...
export function reconcile(effortRoot) {
  ...
  // stores it). A registered WO with no ledger events yet reads `pending`; a `merged`-flagged WO with
  // no fold twin reads `done`. Crash-recovery downgrades below overwrite the ids they settle. Never
  // reads the retired journal `status`.
  const workOrderStatuses = {};
  for (const id of Object.keys(workOrders)) {
    const st = woFold.get(id);
    workOrderStatuses[id] = st ? st.status
      : (workOrders[id] && workOrders[id].merged === true ? 'done' : 'pending');
  }
