...
export function reconcile(effortRoot) {
  ...
  // a later matching `resolvesSeq` folds it to pending and it drops out of this set (closed). Merged
  // terminals are excluded (a merged WO is done, never re-blocked).
  const blockedWorkOrders = [];
  for (const [id, st] of woFold) {
    if (st.status !== 'blocked' || terminalWorkOrders.includes(id)) continue;
