...
import { findEffortRoot, rootFromArgv, readJson, readJsonl, gitTry, norm, loadConfig, samePath, underPath, effortBirthState, assertNoAmbiguousBirth } from './effort.mjs';
...
export function reconcile(effortRoot) {
  ...
  // --- Born-state HALT (S7, §6.1): key the config-trust HALT on effortBirthState. -----------
  // The runMode-absent bucket above catches a CORRUPT config only INCIDENTALLY (loadConfig swallows
  // the parse failure into its defaults → runMode:null → HALT). A MISSING-SIGNATURE config — one that
  // parses and carries a runMode but has no `config.effort` birth signature (foreign / hand-edited /
  // torn) — sails PAST that gate (runMode is present) yet is NOT a config recovery may trust. Key the
  // HALT on the SHARED effortBirthState predicate (the same one discovery uses) so both holes close:
  // `corrupt` AND `missing-signature` are AMBIGUOUS→HALT. `absent` cannot reach here (reconcile already
  // returned {active:false} with no journal, and a born effort has a config), and `ok` never trips it.
  const birth = effortBirthState(effortRoot);
  if (birth.state === 'corrupt' || birth.state === 'missing-signature') {
    const fix = birth.state === 'missing-signature'
      ? `add the effort name — "effort": "<name>" — to .reasonable/config.json (this effort likely predates the birth signature)`
      : `repair the unparseable .reasonable/config.json (${birth.reason ?? 'JSON parse error'})`;
    const what = birth.state === 'missing-signature'
      ? `.reasonable/config.json is missing its birth signature (no non-empty "effort" field)`
      : `.reasonable/config.json is unparseable`;
    ambiguities.push({
      haltReason: `${what} — recovery cannot trust an unidentified config. Fix: ${fix}.`,
      evidence: { effortBirthState: birth.state, reason: birth.reason ?? null },
    });
    notes.push(`config.json birth-state '${birth.state}' → AMBIGUOUS. Fix: ${fix}.`);
  }
