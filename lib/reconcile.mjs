...
//                     order; an absent `config.runMode` on a cold restart (D10).
...
export function reconcile(effortRoot) {
  ...
  const floorBackstopItems = floorSurfaced.map((t) => ({
    ...
    summary: `floor test "${t.id}" changed with no accounting event — backstop tripwire${t.explainedByVerdict ? ' (explained-by-verdict, advisory)' : ''}`,
    status: 'open',
    breaking: true,
  }));
...
export function briefing(r) {
  ...
  if (r.floorIntegrity) {
    ...
    const explained = fi.explainedByVerdict ? `, ${fi.explainedByVerdict} explained-by-verdict (advisory)` : '';
    lines.push(`Floor integrity: ${fi.checked} floor test(s) checked, ${fi.surfaced} unaccounted change(s) SURFACED (backstop tripwire${explained}).`);
  }
