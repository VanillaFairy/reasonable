...
export function atomFootprint(atom, citationGraph) {
...
// ── genesis needs fidelity (DESIGN-3.0 §2.2; reasonable 3.0 A1) ──────────────
//
// The per-atom union the two graph projections use so a live effort's genesis graph is NON-EMPTY and
// stays correct as deltas land — no mode flag. A SPEC'D atom (has deltaClauses) contributes ACTUAL
// needs (needsEdges reads its delta citations); an UN-SPEC'D chartered atom contributes PLANNED needs
// (plannedNeedsEdges reads its charter premises). The partition is by SOURCE: needsEdges only ever emits
// from atoms that HAVE deltaClauses, so dropping planned edges whose `from` is spec'd leaves the two
// fidelities disjoint by source — the union needs no dedup and never double-counts. At pure genesis (all
// un-spec'd) this reduces to plannedNeedsEdges; at full-spec, to needsEdges. Both edge sets share the
// {from,to,edge:'needs',op:'add'} shape (§2.2 — planned vs actual is which function produced the array).
export function needsEdgesWithPlanned(atoms) {
  const list = atoms || [];
  const specced = new Set(list.filter((a) => (a.deltaClauses || []).length > 0).map((a) => a.id));
  const planned = plannedNeedsEdges(list).filter((e) => !specced.has(e.from));
  return [...planned, ...needsEdges(list)];
}
...
export function foldAsLived(effortRoot, { uptoSeq } = {}) {
  ...
  // foldAsLived stays LEDGER-ONLY, self-sufficient (its documented invariant, test-enforced): the
  // containment tree is flat here — the ratified ownership map is an on-disk genesis artifact read only
  // by the live projection (deriveCurrent) below. needs uses the planned+actual fidelity because planned
  // edges derive from charter PREMISES, which ARE ledger events, so foldAsLived stays self-sufficient.
  const containment = containmentTree(atoms);
  const graph = ledgerCitationGraph(atoms);
  const edges = [
    ...needsEdgesWithPlanned(atoms),
...
export function deriveCurrent(effortRoot, { goals = [], spikeInforms = [] } = {}) {
  ...
  const folded = foldAtomsFromEvents(events);
  const atoms = Object.values(folded);
  // The live/current view nests atoms under the ratified component -> subeffort ownership map (Gap D —
  // the id-duality collapse); absent ownership.json degrades to the flat one-level tree. needs uses the
  // planned+actual fidelity so a genesis effort (charters only, no deltas) still has a non-empty graph.
  const containment = containmentTree(atoms, { ownershipMap: readOwnership(effortRoot).ownership || undefined });
