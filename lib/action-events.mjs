...
//
// Every appended event also carries a `dispatch` field: the work order's monotonic dispatchEpoch,
// stamped by lib/action-report.mjs (read from the journal, NOT supplied by the agent). It is
// script-authoritative provenance — "which dispatch of this work order emitted this" — so it is
// not validated here (the script always produces a valid non-negative integer, and an agent
// cannot spoof it: reportAction overrides any incoming field of that name). The projection
// (lib/progress.mjs) reads it to tell a same-dispatch re-affirmation (idempotent no-op) from a
// resumed run's reopen (a crash boundary). See docs/artifacts.md (journal.json) + DESIGN §D19.
