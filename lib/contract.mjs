...
//
// NOTE — two unrelated uses of the word "seam", kept disjoint by context:
...
//   • the `## Observable Seams` SECTION (below) is the PUBLIC test-observation surface of
//     a render-only clause (the export to import + a stable test handle per element). It
//     is parser-invisible to clauses/citations (the `## Scenarios` property) and is read
//     by the blind-test-writer to TARGET a declared handle instead of guessing one.
...
/** Parse one contract file's text. Returns { component, owner, status, seam, seams, clauses, citations, gates }. */
export function parseContract(text, component) {
  ...
  for (const line of lines) {
    ...
    if (h2) {
      ...
      // "Observable Seams" only — never the bare "Seams"/"Topology"/"Scenarios" headers.
