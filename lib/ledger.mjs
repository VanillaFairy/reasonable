...
// reasonable 3.0 Part 7 (§2.4): append() code-computes an atom-verdict's effect set, which needs the
// effect calculus (rewrite.mjs) over a canonical graph snapshot (graph/contract/goals/policy). Those
// are HIGH-level modules; ledger.mjs is a LOW-level one that clause-id.mjs and atom.mjs import for
// append(). Importing them statically here inverts that dependency and forms an INIT-TIME cycle
// (ledger → graph/contract → clause-id → ledger), which throws before contract.mjs finishes loading
// its top-level CLAUSE_ID_PATTERN. They are needed only at CALL time, inside the atom-verdict branch,
// by which point every module is fully initialized — so acquire them lazily. `require(esm)` is stable
// in Node ≥22 and shares the ESM module cache, so these are the same singletons `import` would bind.
const require = createRequire(import.meta.url);
let _verdictDeps = null;
function verdictDeps() {
  if (!_verdictDeps) {
    const rewrite = require('./rewrite.mjs');
    _verdictDeps = {
      computeVerdictEffects: rewrite.computeVerdictEffects,
      ceremonyEscalation: rewrite.ceremonyEscalation,
      unwindCeremonyEscalation: rewrite.unwindCeremonyEscalation,
      deriveCurrent: require('./graph.mjs').deriveCurrent,
      citationGraph: require('./contract.mjs').citationGraph,
      readGoals: require('./goals.mjs').readGoals,
      readPolicy: require('./policy.mjs').readPolicy,
    };
  }
  return _verdictDeps;
}
...
// §5.6 (F12): amendment/ratification MAY carry a structured drop record and a resolvesSeq closure
// pointer — both OPTIONAL and additive (old events lack them, so they are NEVER required). `drops`,
// when present, is an array of { workOrder:string, supersededBy?:string }; `resolvesSeq`, when
// present, is a POSITIVE INTEGER — a 1-based ledger seq (the seq of the amendment-drop it closes).
// The ledger fold (lib/wo-status.mjs), reconcile's closure surfacing, and redispatch-guard all key on
// these, so their SHAPE is validated here — a malformed drop must never land raw and detonate at fold time.
function validateDropsAndResolvesSeq(event) {
  if (event.drops !== undefined) {
    if (!Array.isArray(event.drops)) {
      return { ok: false, error: `${event.type}: 'drops', when present, must be an array of { workOrder, supersededBy? }` };
    }
    for (const d of event.drops) {
      if (!d || typeof d !== 'object' || Array.isArray(d) || !isNonEmptyString(d.workOrder)) {
        return { ok: false, error: `${event.type}: each drop must be an object with a non-empty 'workOrder' string` };
      }
      if (d.supersededBy !== undefined && !isNonEmptyString(d.supersededBy)) {
        return { ok: false, error: `${event.type}: drop.supersededBy, when present, must be a non-empty string` };
      }
    }
  }
  if (event.resolvesSeq !== undefined && !(Number.isInteger(event.resolvesSeq) && event.resolvesSeq > 0)) {
    return { ok: false, error: `${event.type}: 'resolvesSeq', when present, must be a positive integer (a 1-based ledger seq)` };
  }
  return undefined;
}
...
// §7.1 (Layer 2): a `next-action` event PERSISTS the deterministic decision projection (T2.2's
// projectDirectives output) into the truth log, so the directive survives the wholesale mirror regen
// BY CONSTRUCTION — the read-side re-derives the latest such event on every regen, never poking it into
// progress.json out of band. Shape:
//   { type:'next-action', directives:[ { kind, slice?, workOrders?, workOrder?, detail? }, … ], computedFrom:<seq> }
// `directives`, when present, is an ARRAY whose every element is an object with a NON-EMPTY-STRING `kind`
// (the rest — slice/workOrders/workOrder/detail — is free-form, projected by next-action.mjs; don't
// over-validate it). `computedFrom`, when present, is a POSITIVE INTEGER (a 1-based ledger seq — the
// latest seq at projection time; an empty-ledger projection has no such seq and simply OMITS the field,
// so 0/absent is never a validation failure, a non-positive value IS). Family 3 (loose): the fold
// (wo-status.mjs) ignores it for WO status, and progress-map.mjs maps it to NO tree op.
function validateNextAction(event) {
  if (event.directives !== undefined) {
    if (!Array.isArray(event.directives)) {
      return { ok: false, error: `${event.type}: 'directives', when present, must be an array of { kind, … }` };
    }
    for (const d of event.directives) {
      if (!d || typeof d !== 'object' || Array.isArray(d) || !isNonEmptyString(d.kind)) {
        return { ok: false, error: `${event.type}: each directive must be an object with a non-empty 'kind' string` };
      }
    }
  }
  if (event.computedFrom !== undefined && !(Number.isInteger(event.computedFrom) && event.computedFrom > 0)) {
    return { ok: false, error: `${event.type}: 'computedFrom', when present, must be a positive integer (a 1-based ledger seq)` };
  }
  return undefined;
}
...
export const EVENT_SCHEMAS = {
  ...
  'concluded': { required: [] },
  'abandoned': { required: [] }, // walked-away twin of 'concluded' — same node-less terminal shape
  ...
  //
  // 'kind' here is a VERDICT kind (checkpoint/oversized/ripple/…, lib/rewrite.mjs VERDICT_KINDS), NOT
  // one of the 2.x KINDS enum. So it is required via a custom `validate` rather than the required-list
  // — listing it in `required` would force it through the KINDS gate above (which checks the 2.x
  // ['work-order','spike',…] set) and wrongly reject every verdict kind. The specific verdict kind is
  // validated for real by append()'s computeVerdictEffects, which HALTs fail-closed on an unknown kind.
  'atom-verdict': {
    required: ['atomId'],
    validate: (e) => (isNonEmptyString(e.kind) ? undefined : { ok: false, error: "atom-verdict: requires a non-empty 'kind' string" }),
  },
  ...
  'commit': { required: [] },
  // Layer 2 (§7.1) — the persisted decision projection. Family 3 (the implicit else), NOT a node
  // lifecycle event: do NOT add it to FAMILY_1_TYPES/FAMILY_2_TYPES below.
  'next-action': { required: [], validate: validateNextAction },
};

const FAMILY_1_TYPES = new Set([
  'node-planned', 'node-dispatched', 'node-checkpointed', 'node-downgraded',
  'node-completed', 'node-failed', 'node-panicked', 'node-canceled', 'approval-resolved', 'concluded', 'abandoned',
]);
...
export function validateEvent(event) {
  ...
  if (event.effects !== undefined) {
    const effectsResult = validateEffects(event.effects);
    if (!effectsResult.ok) return { ok: false, error: `${type}: ${effectsResult.error}` };
  }
...
export function append(root, event, opts = {}) {
  ...
  stamped.ts = localISOString();

  const ledgerPath = join(root, '.reasonable', 'ledger.jsonl');

  // ONE lock hold covers the WHOLE mutating sequence: address/attempt resolution (buildTree), seq
  // assignment + file append, and the mirror regen. It used to cover only the raw append —
  // buildTree()/attempt arithmetic ran BEFORE the lock (§5.4: two concurrent same-node appends
  // read identical pre-append history and stamp the SAME attempt slot) and writeMirror ran AFTER it
  // (§5.3: concurrent appends tear/stale progress.*). Both defects come from work happening outside
  // the lock; the fix is to widen the hold to the whole sequence. withLock is NOT re-entrant, so the
  // lock is taken exactly ONCE here and the inner write uses effort.mjs's lock-free
  // `appendJsonlLocked` (nesting `appendJsonl`'s own withLock would self-deadlock). Nothing slow runs
  // under the hold — buildTree/writeMirror are bounded in-memory folds + local fs reads/writes, no
  // subprocess, no network.
  let outcome;
  try {
    outcome = withLock(ledgerPath, () => {
      if (FAMILY_1_TYPES.has(type)) {
        ...
        if (type === 'node-dispatched' || type === 'node-downgraded') {
          // Attempt = a SIBLING, minted by the controller. The agent sent the base path; we survey
          // the family and either open a fresh/continuing attempt or (on a failed live attempt,
          // dispatch only) mint the next `[k]` sibling. node-downgraded seals the current live
          // attempt. This survey now reads a consistent snapshot under the lock, so a concurrent
          // sibling append can no longer race it to the same slot.
        ...
        if (type === 'atom-verdict') {
          const { computeVerdictEffects, ceremonyEscalation, deriveCurrent, citationGraph, readGoals, readPolicy } = verdictDeps();
        ...
        if (type === 'ratification') {
          ...
          if (ratifies.length > 0 || rejects.length > 0) {
            const { unwindCeremonyEscalation } = verdictDeps();
      ...
      const seq = appendJsonlLocked(ledgerPath, stamped);
      stamped.seq = seq;

      // The ledger line above is already durably written — that's the fact that matters. The mirror
      // regen stays INSIDE the hold so a reader never sees progress.* disagreeing with the just-
      // appended line. It is still BEST-EFFORT: a disk/permission error on progress.json/progress.md
      // must never propagate out of append() and must never be reported as a failed append (the
      // caller has no way to know a retry wouldn't duplicate the write it already made). Surface it
      // as a sibling `mirrorError` on an otherwise-successful result instead.
      let mirrorError;
      if (opts.regen !== false) {
    ...
    // withLock throws only when it never acquired the lock (5000ms deadline exceeded) — the append
    // never happened. append()'s contract is "never throws"; surface it as a normal ok:false.
    return { ok: false, error: `${type}: could not acquire the ledger lock (${e.message})` };
  }
  return outcome;
}
