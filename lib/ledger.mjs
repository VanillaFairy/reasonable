...
// present, is a finite number (the seq of the node-failed / amendment-drop it closes). The ledger
// fold (lib/wo-status.mjs), reconcile's closure surfacing, and redispatch-guard all key on these, so
// their SHAPE is validated here — a malformed drop must never land raw and detonate at fold time.
function validateDropsAndResolvesSeq(event) {
  ...
  if (event.resolvesSeq !== undefined && !(typeof event.resolvesSeq === 'number' && Number.isFinite(event.resolvesSeq))) {
    return { ok: false, error: `${event.type}: 'resolvesSeq', when present, must be a finite number` };
  }
  return undefined;
}
