...
function resolveFamily2(event, tree) {
  ...
  // Guard against a self-nested node id (BUG 4): `event.node` must be a BARE relative segment
  // to append under the resolved parent's attempt folder — never the already-qualified
  // ancestor path, and never an `attempt-N` segment (that folder name is minted ONLY by this
  // function, right below). A caller that echoes back an already-qualified path would
  // otherwise get it silently re-qualified — `${found.path}/attempt-N/${event.node}` doubling
  // the slice/WO/attempt prefix inside itself. Fail loud here so a misbehaving caller can never
  // corrupt the tree; capability over self-report, same principle as the grammar guard below.
  if (typeof event.node === 'string' &&
      (event.node === found.path || event.node.startsWith(`${found.path}/`) ||
       /(^|\/)attempt-\d+(\/|$)/.test(event.node))) {
    return {
      ok: false,
  ...
  const node = `${found.path}/attempt-${attempt}/${event.node}`;
  // Same grammar guard as Family 1's direct-node path: a relative segment with whitespace or
  // control characters must fail loud HERE, not land raw in the ledger and detonate at fold
  // time. findByPath is used purely as the grammar check — a miss (null) is fine, the report's
  // inject creates the node.
