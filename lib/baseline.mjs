...
//                         accounted event explains is AMBIGUOUS, and the byte-
//                         level hash is now a BACKSTOP TRIPWIRE (D6): an
//                         `explained-by-verdict` annotation never flips it to
//                         silently-accounted — the diff still SURFACES; the
//                         caller demotes from a first-line HALT to a backstop
//                         that still queues to the human (annotate-not-disarm).
//
 ...
 *     changed, accounted, ambiguous, explainedByVerdict }
 *
 ...
 *   - `ambiguous` : `changed && !accounted` — an UNACCOUNTED floor change.
 *                   The caller now treats this as a BACKSTOP TRIPWIRE that still
 *                   SURFACES (and, in autonomous mode, still queues to the
 *                   human), not a first-line HALT (D6 demotion).
 *   - `explainedByVerdict` : true if an `accept` `verifier-verdict` event names
 *                   this floor test / its locus — an ADVISORY annotation only
 *                   (D6, annotate-not-disarm). It NEVER clears `changed` or
 *                   `ambiguous`: a byte-level diff stays surfaced, the verdict
 *                   merely explains it. A missing/half-written verdict can only
 *                   cause MORE surfacing, never less — the failure direction is
 *                   toward scrutiny.
 *
...
export function floorIntegrity(effortRoot) {
  ...
  return baseline.floor.map((entry) => {
    ...
    // The verifier-verdict annotation is ADVISORY (D6): it explains a diff but
    // does NOT account it away. `changed`/`ambiguous` are computed WITHOUT it, so
    // an explaining verdict can never suppress surfacing.
    const explainedByVerdict = changed && locusIsExplained(entry, accounted);
    return {
      id: entry.id,
      locus: entry.locus,
      files,
 ...
 *
 * The `verifier-verdict` event (D5/D6) is collected SEPARATELY into the
 * `explainIds`/`explainGlobs` sets. It is an ADVISORY annotation, NOT an
 * accounting kind: it must NEVER feed `ids`/`globs` (which drive `accounted` and
 * therefore clear `ambiguous`), or an `accept` verdict would silence the
 * floor-integrity hash — exactly the annotate-not-disarm property D6 forbids.
 * Only an `accept` verdict explains; `reject`/`escalate` annotate nothing.
 */
function accountedLoci(effortRoot) {
  ...
  const explainIds = new Set();   // floor test ids an ACCEPT verifier-verdict annotates (advisory)
  const explainGlobs = new Set(); // loci/paths an ACCEPT verifier-verdict annotates (advisory)
  for (const e of ledger) {
    if (!e || typeof e !== 'object') continue;
    // verifier-verdict (D5): advisory-only EXPLAINS marker — kept off ids/globs.
    if (e.type === 'verifier-verdict') {
      if (String(e.verdict || '').toLowerCase() !== 'accept') continue;
      for (const k of ['test', 'floorTest', 'component', 'supersedes']) addAll(explainIds, e[k]);
      if (Array.isArray(e.tests)) for (const t of e.tests) explainIds.add(String(t));
      for (const k of ['locus', 'seam', 'paths', 'diffRef']) addAll(explainGlobs, e[k]);
      continue;
    }
  ...
  return { ids, globs, explainIds, explainGlobs };
}
...
function locusIsAccounted(entry, accounted) {
  return matchesEntry(entry, accounted.ids, accounted.globs);
}

/**
 * Is this floor entry's change EXPLAINED by an `accept` verifier-verdict (D6)?
 * Same matching shape as `locusIsAccounted`, but read off the separate advisory
 * sets so it cannot influence `accounted`/`ambiguous`. Used purely to set the
 * `explainedByVerdict` annotation — the diff still surfaces regardless.
 */
function locusIsExplained(entry, accounted) {
  return matchesEntry(entry, accounted.explainIds, accounted.explainGlobs);
}

/**
 * Shared matcher: does this floor entry's id, pinned files, or locus fall under
 * one of the given id / glob sets? Glob-vs-path matching is bidirectional.
 */
function matchesEntry(entry, idSet, globSet) {
  if (idSet.has(entry.id)) return true;
  const files = pinnedFiles(entry);
  const declared = [...globSet];
  // A declared glob/path covers one of this entry's pinned files.
  if (files.some((f) => matchesAny(f, declared))) return true;
  // A declared concrete path falls under one of this entry's locus globs.
