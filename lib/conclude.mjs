...
// script-stamped and the progress mirror folds the "concluded" event to root status "done"
// immediately (regen defaults ON) — this IS the whole point of the command, so a failure here
// is fatal: nothing gets archived over a ledger that doesn't actually say "concluded".
const ledgered = append(effortRoot, { type: 'concluded', effort });
if (!ledgered.ok) {
  console.error(`reasonable: FAILED to record "concluded" in the ledger for "${effort}": ${ledgered.error}`);
  console.error('The effort is NOT concluded — nothing was committed or archived.');
  process.exit(1);
}
