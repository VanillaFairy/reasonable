...
// reconcile.mjs so it has a home and a test (it was a private, untested function
// whose rich output the reconciler agent flattened in prose). Trust is earned,
// persistent, EVENT-invalidated: a trusted-green test is re-verified only when its
// governing clause is amended or its behavior extended SINCE that test's last
// verification — no re-checking churn. The ledger IS the event log; the mapping
// (test ↔ clause) is the contract's citation, mechanical not eyeballed (DESIGN §5
// D13; architecture S16).
//
// Law 1 (dependency-free): node builtins only — and in fact NO I/O at all. The
// caller reads the ledger; trustStaleness() is pure over that array. reconcile.mjs
// calls it and threads the DERIVED flat `staleTrusted` id list into the briefing,
// which the reconciler copies verbatim (like terminalWorkOrders) and the audit
// consumes at slice level — no per-work-order re-derivation anywhere.
