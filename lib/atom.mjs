...
export function foldAtomFromEvents(events, atomId) {
  ...
  for (const e of events) {
    ...
    // Effects overlay (DESIGN-3.0 §7.2, §8): lib/ledger.mjs's append() code-computes and stamps a
    // real `.effects` array onto atom-verdict events (provisional, applies immediately) and onto
    // ratification events (permanent, once ratifiesSeqs/rejectsSeqs resolve — already folded into
    // that ratification's OWN `.effects` by lib/ledger.mjs, not by us). Read `.effects` generically
    // off EVERY event, whatever its type — never special-case atom-verdict/ratification — because one
    // event's effects array can address several different atoms at once (e.g. an R2 dead-end verdict
    // whose subject is atom A also freezing neighbor B). Each entry's own `nodeId` — independent of
    // the carrying event's own `atomId` — says which atom it targets, so this must run before (and
    // regardless of) the `e.atomId !== atomId` skip below, gated only on the atom being folded here
    // already existing (`record`). NEVER read `.pendingPermanent` — that is strictly
    // lib/ledger.mjs's concern on the write side. Only two change shapes are lifecycle facts; any
    // other shape ({reprice}, {charter}, an edge effect, ...) is left untouched. Entries are applied
    // in ledger order, so on the same field for the same atom the later entry simply overwrites the
    // earlier one (no extra conflict logic needed).
    if (record && Array.isArray(e.effects)) {
      for (const entry of e.effects) {
        if (!entry || entry.nodeId !== atomId) continue;
        const change = entry.change;
        if (!change || typeof change !== 'object') continue;
        if (typeof change.state === 'string') {
          record.state = change.state;
        } else if (typeof change.flag === 'string' && change.op === 'set' && isValidFlag(change.flag)) {
          record.flags.add(change.flag);
        } else if (typeof change.flag === 'string' && change.op === 'clear' && isValidFlag(change.flag)) {
          record.flags.delete(change.flag);
        }
      }
    }
...
export function validateCharterShape(charter) {
  ...
  if (!Array.isArray(locus) || locus.some((l) => typeof l !== 'string')) {
    return { ok: false, error: 'charter: locus must be an array of strings' };
  }
