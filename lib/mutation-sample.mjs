...
if (!sites.length) { out({ k: K, sampled: 0, ran: 0, survivors: [], note: 'no mutable sites in scope' }); }
...
// `ran` keeps the summary honest (parity): a sampled site whose file no stack owns is skipped,
// so "sampled" alone would overclaim how many mutants were actually verified.
out({ k: K, sampled: sampled.length, ran: sampled.length - unresolved.length, survivors, ...(unresolved.length ? { unresolved } : {}) });

function out(report) {
  // An unresolved site is a FAILING condition, same as a survivor: the check could not verify
  // those mutants at all (docs/artifacts.md: a file matching no stack entry is a LOUD gap).
  // Exiting 0 over a gap would let a gate pass on a check that never ran.
  const gaps = report.unresolved ? report.unresolved.length : 0;
  const bad = (report.survivors && report.survivors.length) || gaps;
  if (asJson) { console.log(JSON.stringify(report, null, 2)); process.exit(bad ? 1 : 0); }
  console.log(`Mutation sampling: ${report.ran} of ${report.sampled} sampled mutants run (k=${report.k}).`);
  if (gaps) {
    console.log(`\nUnresolved (${gaps}) — no config.json testCommand stack owns these mutated files; their mutants were NOT verified:`);
    for (const f of report.unresolved) console.log(`  ${f}`);
  }
  if (report.survivors && report.survivors.length) {
  ...
  if (!bad) { console.log('No survivors — sampled assertions are not vacuous. ✓'); process.exit(0); }
  process.exit(1);
}
