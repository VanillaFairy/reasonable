...
//
// Requires testCommand in .reasonable/config.json.

import { execSync } from 'node:child_process';
import { readFileSync, writeFileSync, readdirSync, statSync } from 'node:fs';
import { join } from 'node:path';
import { findEffortRoot, loadConfig, matchesAny, norm, relative } from './effort.mjs';
...
function testsPass() {
  try { execSync(cfg.testCommand, { cwd: effortRoot, stdio: ['ignore', 'pipe', 'pipe'] }); return true; }
  catch { return false; }
}
...
for (const s of sampled) {
  ...
  try {
    if (testsPass()) survivors.push({ file: s.rel, line: s.li + 1, from: s.original.trim(), to: mutatedLine.trim() });
  } finally {
    writeFileSync(s.file, text); // restore exact original bytes
  }
}

out({ k: K, sampled: sampled.length, survivors });
