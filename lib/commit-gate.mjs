...
function inScope(entry, scope, conservativeFallback = false) {
  const p = entry.path;
  if (p === '.reasonable' || p.startsWith('.reasonable/')) return true; // effort artifacts (always provable)
  if (scope.loci.length && matchesAny(p, scope.loci)) return true;      // inside a declared locus
  // Fallback only: loci unknown. By default a TRACKED modification is presumed effort
  // work; an UNTRACKED file of unknown provenance is left for the human (never swept).
  // conservativeFallback (the unattended Stop backstop) declines even the tracked mod:
  // with no work-orders to prove provenance, it must not sweep a main-checkout change
  // (e.g. a stray .gitignore) onto the human's branch — it surfaces it instead.
  if (scope.source === 'fallback' && !entry.untracked) return !conservativeFallback;
  return false;
}
 ...
 */
export function commitGate(start, { commit = false, message = null, conservativeFallback = false } = {}) {
  ...
  const toStage = entries.filter((e) => inScope(e, scope, conservativeFallback)).map((e) => e.path);
  const leftUntracked = entries
    .filter((e) => e.untracked && !inScope(e, scope, conservativeFallback))
    .map((e) => e.path);
  // Tracked main-checkout mods the conservative backstop declined to sweep in fallback
  // (no work-orders ⇒ provenance unprovable). Surfaced, never committed automatically.
  const leftTracked = (conservativeFallback && scope.source === 'fallback')
    ? entries.filter((e) => !e.untracked && !inScope(e, scope, conservativeFallback)).map((e) => e.path)
    : [];
  ...
  if (leftTracked.length) {
    result.warnings.push(
      `loci undeterminable (no work-orders) — left ${leftTracked.length} tracked main-checkout ` +
      `change(s) unswept (provenance unprovable): ${leftTracked.join(', ')}`,
    );
  }
