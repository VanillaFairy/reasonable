...
import {
  ...
  relative, resolve, readJsonl, roleOf, isUnder,
} from './effort.mjs';
...
const REASONABLE_WRITE_PERMS = {
  CONTRACT: ['implementer', 'characterizer', 'scaffolder', 'census'], // provider-owned clauses + skeletons
  LEDGER: [], // no role may write the ledger directly (2.0) — every append, from any
              // worker or the scribe, goes through the ledger controller CLI instead.
...
function classifyReasonable(rel) {
  ...
  if (r === '.reasonable/ledger.jsonl') return 'LEDGER';
...
function governReasonable(role, rel) {
  ...
  if (cls === 'LEDGER') {
    return {
      decision: 'deny',
      reason:
        `Identity-governed .reasonable/ write denied: role "${role}" may not write the LEDGER artifact "${rel}" ` +
        `directly (DESIGN §5.14 control surface). No role appends to the ledger by Edit/Write/shell redirection ` +
        `anymore — append via 'node lib/ledger.mjs append --root <path> ...' — the ledger controller is the only ` +
        `write path (2.0). Workers propose the event as data; the controller validates, stamps, and persists it.`,
    };
  }
  return {
    decision: 'deny',
...
if (tool === 'Bash') {
  ...
  for (const target of extractWriteTargets(command)) {
    ...
    if (!er) continue; // outside any effort — not policed
    const rel = relTo(er, abs);
    if (!classifyReasonable(rel)) continue; // code/build target outside `.reasonable/` — not policed
...
if (!effortRoot) process.exit(0); // no effort reachable — plain repo / external checkout — allow

const rel = relTo(effortRoot, tgt);

// A canonical `.reasonable/` (or lane-descriptor) write — identity-governed (§5.14).
if (classifyReasonable(rel)) {
