...
const REASONABLE_WRITE_PERMS = {
  ...
  LEDGER: ['implementer', 'characterizer', 'scaffolder', 'journal-writer'], // each worker appends its own event (artifacts.md); the scribe appends verifier-verdicts
  ...
  SEALED: [],                     // config/supervision/vision/route/work-orders/verdicts/knowledge/… — orchestrator only
};
 ...
 * path (config, supervision, vision, route, work-orders/, verdicts/, knowledge/, …)
 * — those are written only by the orchestrator (main session), never a subagent.
 */
