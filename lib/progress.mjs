...
// One ephemeral heartbeat as a human line: "implement · Edit ChoiceEdge.tsx  (3s ago)".
function liveLine(lv) {
  const who = lv.stage || lv.role || '?';
  const what = [lv.tool, lv.target].filter(Boolean).join(' ');
  return `${who}${what ? ` · ${what}` : ''}  _(${fmtAge(lv.ageMs)})_`;
}
...
export function renderMarkdown(m) {
  ...
  L.push(`_${sub}_`);
  L.push('');
  L.push('> Pin this file to follow the run live — it is regenerated from the ledger on every journal update and on every subagent tool call (the ⟳ heartbeats), no model in the loop.');
  L.push('');
