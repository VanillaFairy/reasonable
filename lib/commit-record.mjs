...
// progress-live heartbeat (D19): the heartbeat is a presentation POINTER, this is a recovery ANCHOR.
//
// Usage: node commit-record.mjs --hook   (PostToolUse on Bash; reads the payload on stdin)

import { readStdinJson, findLane, readJsonl, appendJsonl, gitTry, roleOf, join, basename } from './effort.mjs';
...
export function recordCommit(input) {
  ...
  appendJsonl(ledgerPath, { type: 'commit', workOrder: lane.workOrder, commit: sha, role: roleOf(input) || null, by: 'commit-record' });
