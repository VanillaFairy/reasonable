...
export function writeMirror(root) {
  ...
  // SPREAD: the tree object itself plus a `counts` key — never a {tree, counts} wrapper. Each file
  // is published atomically (tmp + rename), same on-disk shape writeJson produced (2-space JSON +
  // trailing newline).
  atomicWrite(join(dir, 'progress.json'), JSON.stringify({ ...tree, counts }, null, 2) + '\n');
  atomicWrite(join(dir, 'progress.md'), composeProgressMd(tree, counts, journal.cost, inboxItems));
