...
// Every PROPER ancestor of `path` (excluding the root and `path` itself) gets nudged to
// 'active', guarded so it only takes while still 'pending'. Without this, a synthetic
// container auto-created along the way to a leaf (an attempt-N folder, an implementation/
// section folder, ...) sits stuck at pending — showing '·' — for as long as the run lasts,
// even while a descendant is visibly '▶ active'. Every node in the tree gets the same
// treatment: a container is never shown less "in motion" than the work happening under it.
function ancestorActivationOps(path) {
  const segments = typeof path === 'string' ? path.split('/') : [];
  const ops = [];
  for (let i = 1; i < segments.length; i++) {
    ops.push({ op: 'status', path: segments.slice(0, i).join('/'), status: 'active', guardPending: true });
  }
  return ops;
}
  ...
  'node-dispatched': (e) => {
    ...
    if (e.attempt > 1) {
      return [
        ...
        { op: 'inject', path: `${e.node}/attempt-${e.attempt}`, label: `attempt ${e.attempt}`, status: 'active' },
        ...ancestorActivationOps(e.node),
        { op: 'status', path: e.node, status: 'active', detail: null, ts: e.ts },
      ];
    }
    return [
      { op: 'inject', path: `${e.node}/attempt-1`, label: 'attempt 1', status: 'active' },
      ...ancestorActivationOps(e.node),
      { op: 'status', path: e.node, status: 'active', detail: null, ts: e.ts },
    ];
  },
