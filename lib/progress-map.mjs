...
export const EVENT_MAP = {
  ...
  'node-completed': (e) => [{ op: 'status', path: e.node, status: 'done', ts: e.ts }],
...
function opsFor(e) {
  const mapper = e && typeof e.type === 'string' ? EVENT_MAP[e.type] : undefined;
...
export function foldEvents(events, rootLabel) {
  const tree = createTree(rootLabel);
  const sorted = [...(events || [])].sort((a, b) => (a?.seq || 0) - (b?.seq || 0));
  for (const e of sorted) {
    try {
      for (const op of opsFor(e)) apply(tree, op);
    } catch (err) {
      try {
        apply(tree, {
          op: 'note',
